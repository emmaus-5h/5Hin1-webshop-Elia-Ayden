...
function getProducts(request, response) {
  ...
  let data = []
  const sqlOpdracht = db.prepare('SELECT products.id AS id, products.name AS name, products.description AS description, products.code AS code, products.price AS price, products.geslacht, maat.maten, categorie.name AS categorie, materiaal.name AS materiaal, recycled.name AS recycled FROM products JOIN maat ON products.maten_id = maat.id JOIN categorie ON products.categorie_id = categorie.id JOIN materiaal ON products.materiaal_id = materiaal.id JOIN recycled ON products.recycled_id = recycled.id ORDER BY products.name')
